...
# Restart Kompewter IRC bot if not currently running
#0,20,40 * * * * ps aux | grep -w kompewter | grep -v 'grep' > /dev/null || /usr/bin/nohup $HOME/kompewter/jenni > $HOME/kompewter/kompewter.log &

# Restart Slack IRC sync bot if not currently running
#0,20,40 * * * * ps aux | grep -w slack-irc | grep -v 'grep' > /dev/null || /usr/bin/nohup /usr/local/bin/slack-irc --config $HOME/slack-irc/config.json > $HOME/slack-irc/slack-irc.log &
